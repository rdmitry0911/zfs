 ...
 * The commit only runs the census + marker; the DDT-tripping rewrite was already
 * done offline. Default 0 = keep the fail-closed guard.
 */
...
typedef struct spa_reparity {
	...
	uint64_t sr_unsupported;	/* R07: gang (or other) blocks we refuse */
	volatile uint32_t *sr_cancelp;	/* R06: cancel flag checked inside the sweep */
	/* bounded force-condense (audit debt): a persistent (vd,ms) cursor over a
	 * flat metaslab enumeration, a per-txg batch budget, and a SURFACED load
	 * error so a metaslab that cannot be loaded fails the op instead of being
	 * silently left un-condensed. */
	uint64_t sr_condense_flat;	/* next flat metaslab index to process */
	...
	boolean_t sr_condense_more;	/* more metaslabs remain after this batch */
	int sr_condense_err;		/* first metaslab_load() error (fail-closed) */
} spa_reparity_t;
    ...
    uint64_t target, uint64_t T, dmu_tx_t *tx)
{
	...
	if (oldn > 0 && vdrz->vd_parity_epochs[(oldn - 1) * 3 + 2] == target)
		return;	/* idempotent: last epoch already AT target (promote or demote) */
	...
	if (oldn == 0) {
		...
		for (uint64_t i = 0; i < oldn * 3; i++)
			nt[i] = vdrz->vd_parity_epochs[i];	/* immutable prefix */
		nt[oldn * 3 + 0] = T;
		nt[oldn * 3 + 1] = W;
		nt[oldn * 3 + 2] = target;
	}
	VERIFY0(zap_update(spa->spa_meta_objset, vd->vdev_top_zap,
	    VDEV_TOP_ZAP_RAIDZ_PARITY_EPOCHS, sizeof (uint64_t), newn * 3, nt, tx));
...
spa_reparity_epochs_sync(void *arg, dmu_tx_t *tx)
{
	...
	if (sr->sr_demote) {
		/*
		 * P11/D: LOWER the advertised tolerance NOW -- in the same txg that
		 * publishes the demotion epoch, before the sweep can write any
		 * weaker-parity block. For demotion the safe direction is early
		 * (understating tolerance is always safe); a crash mid-sweep then still
		 * imports advertising the target, never the old higher guarantee.
		 */
		vdev_raidz_t *vdrz = vd->vdev_tsd;
 ...
 * force-condense: metaslab_condense() truncates and rewrites the ENTIRE spacemap
 * (a superset of a flush -- it folds in the unflushed log entries) at the
 * current (>= start) txg, so every rewritten block lands at the target parity;
 * once every metaslab is flushed the drained log-spacemap objects are freed too.
 *
 ...
 *   (1) spa_reparity_condense_load  -- OPEN context: load one batch, advance the
 *       cursor, record any load error, set sr_condense_more if work remains.
 ...
 *       ms_condense_wanted and dirty the metaslab for the syncing txg; that txg's
 *       vdev_sync -> metaslab_sync (sync pass 1) then condenses it.
 */
static void
	...
	for (int c = 0; c < rvd->vdev_children; c++) {
		...
		for (uint64_t m = 0; m < vd->vdev_ms_count; m++, flat++) {
			if (flat < sr->sr_condense_flat)
				continue;		/* processed in a prior batch */
 ...
 */
/*
 * IDEMPOTENCY: a block is already at the target parity iff its PHYSICAL birth is
 * at or after the promotion epoch's start txg. Skipping such blocks makes the
 * multi-pass sweep (up to 8 passes) idempotent -- pass 0 promotes the parity-1
 * blocks, and every later pass skips them instead of re-rewriting (which would
 * pile generations of old blocks into the deadlist -> head-deadlist bloat ->
 * dsl_dataset_recalc_head_uniq panic on a later snapshot destroy).
 */
static boolean_t
    ...
    volatile uint32_t *cancelp)
{
	...
	for (uint64_t base = 0; base < cap; base += chunk) {
		/*
		 * R06: honour cancellation between chunk transactions so a huge or
		 * sparse object cannot hold the (online) suspend window open until
		 * the whole object is swept. The per-chunk 4 MiB tx already bounds
		 * each transaction; this bounds the loop.
		 */
...
spa_reparity_ds_cb(const char *dsname, void *arg)
{
	...
	if (online && getzfsvfs(dsname, &zfsvfs) == 0) {
		/*
		 * Online, INCREMENTAL: suspend the mounted fs, sweep only a small
		 * batch of objects, resume -- repeat until done. Apps see brief
		 * pauses (one batch each) instead of a single long suspend. The
		 * object cursor persists across cycles (object ids are stable);
		 * concurrent writes between cycles land at the new parity anyway.
		 */
		zfs_vfs_rele(zfsvfs);
		uint64_t cursor = 0;
		for (;;) {
			...
			while (dmu_object_next(os, &obj, B_FALSE, 0) == 0) {
				reparity_sweep_object(os, obj, sr != NULL ? sr->sr_start_txg : 0,
	...
	/* Offline (or online + not mounted): exclusive own, sweep all, disown. */
	objset_t *os;
...
spa_reparity_bp_needs_promo(spa_t *spa, const blkptr_t *bp, uint64_t target)
{
	...
	for (int d = 0; d < SPA_DVAS_PER_BP; d++) {
		 ...
		 * the traverse census callback does not hold it. The vdev tree is
		 * stable during reparity (no add/remove), so the DVA's vdev id is a
		 * valid index into rvd->vdev_child[].
		 */
    ...
    const zbookmark_phys_t *zb, const dnode_phys_t *dnp, void *arg)
{
	...
	if (sr->sr_multi) {
		...
		/*
		 * Direction-aware completion census (P11/D). Promotion is done when no
		 * block is BELOW target; demotion is done when no block is ABOVE target
		 * (every over-parity block re-encoded down so its extra column space is
		 * reclaimable). "parity < target" alone is trivially satisfied for a
		 * demotion (old P3 already satisfies P2) and would falsely certify.
		 */
...
spa_reparity_commit_sync(void *arg, dmu_tx_t *tx)
{
	...
	vdrz->vd_reparity_parity = sr->sr_target;
	/*
	 * P11/D: for DEMOTION the marker + completion_txg were set early (at epoch
	 * publish, when tolerance dropped). Do NOT advance completion_txg to the
	 * commit txg -- a rewind to mid-sweep must still see the marker as covered
	 * (comp <= ub) and advertise the LOWER tolerance, since weaker blocks may
	 * already exist. Only a promotion advances completion to the commit txg
	 * (tolerance rises only once every block reached the higher parity).
	 */
	if (!sr->sr_demote)
		vdrz->vd_reparity_completion_txg = dmu_tx_get_txg(tx);
	vdrz->vd_reparity_epoch_digest = spa_reparity_digest(
 ...
 * for crash-resume). All vdevs share one boundary txg T+1 (the P9 fence: a write
 * racing the flip is classified old-parity and swept). Each vdev keeps its own
 * width W and base parity.
 */
static void
    ...
    reparity_async_t *ra, nvlist_t *onvl)
{
	...
	/*
	 * Fail-closed on features whose shared/refcounted blocks the reparity CoW
	 * rewrite cannot safely re-emit yet: dedup (DDT) and block cloning (BRT).
	 * Rewriting a deduped or cloned block through the normal write path trips
	 * the DDT/BRT accounting (an spl_panic in the zio write pipeline was
	 * observed), so refuse up front rather than wedge or corrupt. The
	 * feature-aware rewrite each needs is specified in rrmvp/feature_reshape.py
	 * (Priority 8b) and is not yet implemented in-kernel.
	 */
	...
	/*
	 * Collect the top-level raidz vdevs. One is the common case (single-vdev
	 * path, unchanged); several are promoted together (multi-vdev, Priority
	 * 8b) provided they share a base parity so a single target applies. Every
	 * raidz vdev must carry its per-vdev top ZAP (for the epoch table).
	 */
	...
	for (int c = 0; c < rvd->vdev_children; c++) {
		...
		if (tvd == NULL)
			...
			/* mixed base parity across vdevs: one target cannot apply. */
			spa_close(spa, FTAG);
			return (SET_ERROR(ENOTSUP));
		}
		nraidz++;
	...
	/*
	 * Base = the last COMMITTED effective parity (the vd_reparity_parity marker),
	 * not the structural vd_nparity and NOT the last appended epoch. After a
	 * committed demotion to 1 the marker is 1, so a re-promotion to 2 is a real
	 * op (target>base). Using the last epoch would be wrong: a REFUSED/incomplete
	 * promotion already appended its {T:target} epoch without committing, so
	 * re-running would see base==target and reject a legitimate resume. With no
	 * marker yet (marker==0) the base is the structural vd_nparity.
	 */
	 ...
	 * base is a no-op and rejected. Both directions need target in [1,MAXPARITY]
	 * and width > target (a raidz vdev needs > parity data columns). */
	sr.sr_demote = (sr.sr_target < sr.sr_base);
	if (sr.sr_target == sr.sr_base || sr.sr_target < 1 ||
	    sr.sr_target > VDEV_RAIDZ_MAXPARITY || sr.sr_width <= sr.sr_target) {
		spa_close(spa, FTAG);
		return (SET_ERROR(EINVAL));
	}
	...
	if (multi) {
		/* every raidz vdev must be wide enough for the target parity. */
		for (int c = 0; c < rvd->vdev_children; c++) {
			vdev_t *cvd = rvd->vdev_child[c];
			if (cvd->vdev_ops == &vdev_raidz_ops &&
	 ...
	 * re-appending with a newer start-txg would re-classify them as residual
	 * and the census could never converge (F5 crash-resume correctness).
	 */
	if (multi) {
		/*
		 * Multi-vdev: establish (or reuse, per-vdev) an epoch on every
		 * top-level raidz vdev in one sync task. Idempotent, so crash-resume
		 * re-runs harmlessly.
		 */
		err = dsl_sync_task(spa_name(spa), NULL,
		    spa_reparity_epochs_multi_sync, &sr, 3, ZFS_SPACE_CHECK_NORMAL);
		if (err != 0) { spa_close(spa, FTAG); return (err); }
	} else {
	boolean_t have_epoch = (vdrz->vd_parity_epoch_count > 0 &&
	    vdrz->vd_parity_epochs[(vdrz->vd_parity_epoch_count - 1) * 3 + 2] >=
	    sr.sr_target);
	if (!have_epoch) {
		err = dsl_sync_task(spa_name(spa), NULL, spa_reparity_epochs_sync,
		    &sr, 3, ZFS_SPACE_CHECK_NORMAL);
		if (err != 0) { spa_close(spa, FTAG); return (err); }
	} else {
		/*
		 * Reuse the existing promotion epoch (e.g. established by
		 * `zhack raidz_epochs` before an offline snap_bpr, or by a prior
		 * reparity pass). Set sr_start_txg to its start txg so the
		 * idempotent sweep skips blocks already promoted (physical birth
		 * >= start), instead of re-rewriting them across the 8 passes.
		 */
		for (uint64_t i = 0; i < vdrz->vd_parity_epoch_count; i++) {
	 ...
	 * rewrites) is rebuilt at the target parity, and the drained log-spacemap
	 * objects are freed. Without this an untouched metaslab's pre-epoch
	 * spacemap block is residual on every census pass -> EAGAIN. BOUNDED: each
	 * iteration loads+dirties at most sr_condense_budget metaslabs from a
	 * persistent cursor; a metaslab_load() failure is surfaced and fail-closes
	 * the op; a second wait per batch lets the freed log spacemaps be reclaimed.
	 */
	sr.sr_condense_flat = 0; sr.sr_condense_err = 0;
	sr.sr_condense_budget = reparity_condense_budget;	/* P10: tunable */
	do {
	...
	{
		/*
		 * Sweep every dataset. Offline: exclusive dmu_objset_own. Online:
		 * the cb suspends each mounted fs (zfs_suspend_fs), sweeps it, then
		 * resumes -- the mount persists (no unmount).
		 */
	...
	if (ra != NULL) { ra->ra_visited = sr.sr_visited; ra->ra_residual = sr.sr_residual; }
	...
	if (err != 0 || sr.sr_residual != 0 || sr.sr_visited == 0 ||
	    sr.sr_unsupported != 0) {
		/* R07: gang (or other unsupported) blocks -> refuse, no commit. */
		fnvlist_add_boolean_value(onvl, "committed", B_FALSE);
		spa_close(spa, FTAG);
		return (err != 0 ? err :
		    (sr.sr_unsupported != 0 ? SET_ERROR(ENOTSUP) : SET_ERROR(EAGAIN)));
	}
...
zfs_ioc_pool_reparity(const char *pool, nvlist_t *innvl, nvlist_t *onvl)
{
	...
	/*
	 * R03: cancel/status are scoped to the requesting pool. The single global
	 * slot records ra_pool; a request for a DIFFERENT pool must never observe
	 * or cancel another pool's operation.
	 */
	if (status) {
		...
		if (!mine) {
			/*
			 * No operation for THIS pool -- do not leak another pool's
			 * state (and leave "pool" unset so the CLI reports "no
			 * reparity operation on record").
			 */
		...
		fnvlist_add_boolean_value(onvl, "committed", ra->ra_committed != 0);
		fnvlist_add_boolean_value(onvl, "canceling", ra->ra_cancel != 0);
	...
	/*
	 * R04: sync and async share ONE gate. Both claim ra_active via CAS, so a
	 * sync call cannot start a second engine while any reparity is active, and
	 * ra_pool identifies the owner for status/cancel.
	 */
...
ZFS_MODULE_PARAM(zfs, reparity_, reached_phase, INT, ZMOD_RW,
	"native crash campaign: highest reparity phase reached (harness polls)");
...
ZFS_MODULE_PARAM(zfs, reparity_, allow_dedup, INT, ZMOD_RW,
	"P8b: permit reparity COMMIT on a dedup/BRT pool whose DDT was migrated "
	"by the dedup-aware BPR (default 0 = fail-closed)");
