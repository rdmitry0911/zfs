		...
		spa_feature_incr(spa, SPA_FEATURE_RAIDZ_PARITY_EPOCHS, tx);
	{
		...
		if (vdrz2->vd_parity_epochs != NULL)
			kmem_free(vdrz2->vd_parity_epochs,
			    vdrz2->vd_parity_epoch_count * 3 * sizeof (uint64_t));
...
zhack_reparity_objset(objset_t *os)
{
	...
	while (dmu_object_next(os, &obj, B_FALSE, 0) == 0) {
		 ...
		 * directories -- with dmu_buf_will_dirty (normal write path so the epoch table selects
		 * block at the new parity while PRESERVING its logical birth, so
		 * it does not trip user accounting (unlike will_dirty). Bounded
		 * 4 MiB tx chunks cover the whole object.
		 */
...
zhack_do_reparity(int argc, char **argv)
{
	...
	do {
		VERIFY0(dsl_sync_task(spa_name(spa), NULL,
		    zhack_reparity_mos_sync, NULL, 128, ZFS_SPACE_CHECK_NORMAL));
		zhack_mos_batches++;
	...
	if (zhack_mos_errors != 0) {
		(void) fprintf(stderr, "reparity: MOS sweep errors; aborting\n");
		spa_close(spa, FTAG);
		return (1);
	}
	/*
	 * The sweep re-emits ZPL objects (files AND directories) with
	 * dmu_buf_will_rewrite. zhack's default ZPL stub aborts on any ZPL-object
	 * dirty; register a REAL file-info callback (zhack_get_file_info, which
	 * parses uid/gid/gen from the znode/SA bonus with no mounted ZPL) so user
	 * accounting stays correct as the extra parity space is attributed to each
	 * object's real owner.
	 */
	...
	for (int i = 1; i < argc; i++) {
		...
		/*
		 * Free any stale intent-log chain (parity-1 ZIL blocks born before
		 * the promotion). The pool is exported/quiesced, so the ZIL is
		 * already committed -- destroying it loses nothing and leaves an
		 * empty header, so no old-parity ZIL block remains in the tree.
		 */
    ...
    const zbookmark_phys_t *zb, const dnode_phys_t *dnp, void *arg)
{
	...
	if (zhack_epoch_parity_for(BP_GET_PHYSICAL_BIRTH(bp)) <
		...
		if (zhack_reparity_residual <= 8 && zb != NULL)
			(void) fprintf(stderr, "  residual: objset=%llu obj=%llu "
...
zhack_do_reparity_commit(int argc, char **argv)
{
	...
	if (zhack_target_parity <= vdrz->vd_nparity)
		fatal(spa, FTAG, "epoch table has no parity promotion to commit");
	...
	if (err != 0 || zhack_reparity_residual != 0 ||
	    zhack_reparity_visited == 0) {
		(void) fprintf(stderr, "reparity_commit REFUSED: traverse_err=%d "
		    "residual=%llu visited=%llu (a skipped/errored subtree or an "
...
zhack_commit_raw_sync(void *arg, dmu_tx_t *tx)
{
	...
	if (vd == NULL || vd->vdev_ops != &vdev_raidz_ops)
		fatal(spa, FTAG, "vdev guid %llu is not raidz", (u_longlong_t)c->guid);
	vdev_raidz_t *vdrz = vd->vdev_tsd;
 ...
 * property governs it); pool metadata follows a separate policy and is excluded.
 */
    ...
    const zbookmark_phys_t *zb, const dnode_phys_t *dnp, void *arg)
{
	...
	if (BP_GET_LEVEL(bp) != 0 || BP_GET_TYPE(bp) != DMU_OT_PLAIN_FILE_CONTENTS)
		return (0);
	zhack_norm_visited++;
	...
	/*
	 * Explicit raw fallback: an incompressible block legitimately stored OFF
	 * conforms to any target compressor -- policy=lz4 must not force us to
	 * falsely tag incompressible bytes as lz4 (auditor RC0). Any OTHER
	...
	if (ck_ok && cp_ok) {
		...
		if (zhack_norm_residual <= 8 && zb != NULL)
			(void) fprintf(stderr, "  non-normalized: objset=%llu obj=%llu "
			    "blkid=%llu checksum=%llu(want %llu) compress=%llu(want %llu)\n",
			    (u_longlong_t)zb->zb_objset, (u_longlong_t)zb->zb_object,
			    (u_longlong_t)zb->zb_blkid, (u_longlong_t)cksum,
			    (u_longlong_t)zhack_norm_target_checksum, (u_longlong_t)comp,
...
zhack_do_normalize_verify(int argc, char **argv)
{
	...
	zhack_spa_open(argv[0], B_FALSE, FTAG, &spa);
	zhack_norm_visited = 0; zhack_norm_conforming = 0; zhack_norm_residual = 0;
	int err = traverse_pool(spa, 0, TRAVERSE_PRE,
	    zhack_normalize_verify_cb, NULL);
	(void) fprintf(stderr, "normalize census: data_blocks=%llu conforming=%llu "
	    "non_normalized=%llu traverse_err=%d target_checksum=%llu "
	    "target_compress=%llu\n", (u_longlong_t)zhack_norm_visited,
	    (u_longlong_t)zhack_norm_conforming, (u_longlong_t)zhack_norm_residual,
	    err, (u_longlong_t)zhack_norm_target_checksum,
	    (u_longlong_t)zhack_norm_target_compress);
	int rc = 0;
	if (err != 0 || zhack_norm_residual != 0 || zhack_norm_visited == 0) {
		(void) fprintf(stderr, "normalize_verify FAILED-CLOSED: traverse_err=%d "
		    "non_normalized=%llu data_blocks=%llu\n", err,
		    (u_longlong_t)zhack_norm_residual, (u_longlong_t)zhack_norm_visited);
		rc = 1;
	} else {
		(void) fprintf(stderr, "normalize_verify OK: all %llu file-data blocks "
		    "at target codec\n", (u_longlong_t)zhack_norm_visited);
	}
	spa_close(spa, FTAG);
	return (rc);
	...
	for (int d = 0; d < SPA_DVAS_PER_BP; d++) {
		...
		if (DVA_GET_VDEV(dva) == zhack_pl_excl_vdev) {
			...
			if (zhack_pl_residual <= 8 && zb != NULL)
				(void) fprintf(stderr, "  mis-placed: objset=%llu obj=%llu "
...
zhack_do_placement_verify(int argc, char **argv)
{
	...
	(void) fprintf(stderr, "placement census: data_blocks=%llu mis_placed=%llu "
	    "traverse_err=%d excluded_vdev=%llu\n", (u_longlong_t)zhack_pl_visited,
	    (u_longlong_t)zhack_pl_residual, err, (u_longlong_t)zhack_pl_excl_vdev);
	int rc = 0;
	if (err != 0 || zhack_pl_residual != 0 || zhack_pl_visited == 0) {
		(void) fprintf(stderr, "placement_verify FAILED-CLOSED: traverse_err=%d "
		    "mis_placed=%llu data_blocks=%llu\n", err,
		    (u_longlong_t)zhack_pl_residual, (u_longlong_t)zhack_pl_visited);
		rc = 1;
	} else {
		(void) fprintf(stderr, "placement_verify OK: no file-data on vdev %llu "
		    "(%llu blocks checked)\n", (u_longlong_t)zhack_pl_excl_vdev,
		    (u_longlong_t)zhack_pl_visited);
	}
	spa_close(spa, FTAG);
 ...
 *
 * RESULT (validated in VM): this CORRUPTS the pool -- a subsequent `zpool import`
 * hangs in D-state at spl_panic() inside dsl_dataset_space_written_impl()
 * (dataset_list_next -> objset_stats -> dsl_get_written): the "written" property
 * computation VERIFYs birth-txg ordering, and a re-anchored creation_txg makes
 * it panic. Recovery required `reboot -f` + deleting the pool images without
 * importing. This proves ZFS's birth-txg is a LOAD-BEARING invariant enforced by
 * kernel assertions, not a soft field: a snapshot cannot be made to own blocks
 * born after its creation by any txg-field surgery. See
 ...
 * counts blocks shared with (already relocated by) an older dataset. For a clone
 * processed after its origin, memo-miss == clone-private, so this is exactly the
 * amount by which the clone's dsl_dir dd_used_bytes must grow (the BPR enlarged
 * the private blocks but block_born/kill are skipped, so dd_used is otherwise
 * stale -> destroying the clone transfers a too-small dd_used to dp_free_dir
 * while the async drain credits the real (larger) size, underflowing `freeing`).
 */
...
/* P8b: count of BRT (block-cloning) entries migrated old-DVA -> relocated new-DVA. */
 ...
 * a shared block would be read/freed twice across snapshots (double-free abort).
 */
typedef struct {
 ...
 * A clone's tree still points at its origin snapshot's ORIGINAL blocks; adding it
 * to the snap_bpr passes (processed oldest-first, after its origin) relocates the
 * clone's tree through the SAME rewrite-once memo, so it points at the relocated
 * blocks -- otherwise reparity fail-closes on the un-promoted shared blocks.
 */
static int
zhack_bpr_collect_clone(const char *name, void *arg)
{
	...
	if (dsl_dataset_hold(dp, name, FTAG, &ds) == 0) {
		...
		/*
		 * A REAL clone's origin is a user snapshot; a normally-created dataset's
		 * dd_origin_obj points at the pool's hidden $ORIGIN snapshot (shared by
		 * the root + all plain filesystems). Only add real clones.
		 */
...
zhack_bpr_bp(spa_t *spa, dmu_tx_t *tx, blkptr_t *bp, const zbookmark_phys_t *zb)
{
	...
	if (mfound != NULL) {
		/* this snapshot references a block relocated for an earlier one;
		 * its physical footprint still grows, so account the delta. */
		zhack_bpr_ref_delta += (int64_t)bp_get_dsize_sync(spa, &mfound->bm_newbp)
	...
	/*
	 * Mirror the physical_rewrite finalize (zio.c zio_dva_allocate): preserve
	 * the LOGICAL birth, stamp PHYSICAL birth = this txg, and set the REWRITE
	 * flag. The flag is what makes BP_GET_BIRTH() return the physical birth for
	 * this block (allocation/scan/born see "born now"), while
	 * BP_GET_LOGICAL_BIRTH() still returns the original (deadlist/destroy
	 * ownership, re-keyed to logical, see the true snapshot birth).
	 */
	BP_SET_BIRTH(&newbp, logical, txg);
	BP_SET_REWRITE(&newbp, 1);
	/*
	 * P8b: preserve the DEDUP bit. The write above used zp_dedup=B_FALSE so
	 * arc_write allocated a FRESH block (not deduped back onto the old DVA), but
	 * the relocated block must stay DDT-managed: its DDT entry is repointed to
	 * this new DVA (zhack_bpr_ddt_remap_sync) and its N logical references (all
	 * resolving to this one new BP via the rewrite-once memo) must free through
	 * ddt_phys_decref so the block is freed only when the last reference drops.
	 * With DEDUP=0 the first free would release the shared block under the other
	 * references -> corruption. Non-dedup blocks keep DEDUP=0 (no-op).
	 */
	...
	newbp.blk_fill = fill;
	{
		...
		zhack_bpr_private_delta += _pd;	/* memo MISS: this dataset's own block */
	}
	*bp = newbp;
	 ...
	 * rewrite). We repoint the BP, record the relocation in the shared memo,
	 * and -- crucially -- a later pass remaps EVERY other structure that also
	 * holds this old BP (the per-snapshot deadlists) through the same memo, so
	 * nothing is left pointing at the old DVA. The old block's space is
	 * reclaimed by the deferred-free pass. The memo relocates a shared block
	 * exactly once (oldest owner); every other reference resolves via memo.
	 */
	zhack_bpr_rewritten++;
...
zhack_snap_bpr_sync(void *arg, dmu_tx_t *tx)
{
	 ...
	 * feature must be active on this snapshot's dataset (same activation the
	 * live rewrite path does in dbuf_sync_leaf). Do it once per snapshot.
	 */
	 ...
	 * ds_referenced) holds. ds_unique is left to the recalc / next-snap freeze
	 * (bumping it by the full delta would over-count blocks shared with a
	 * neighbour); referenced-only keeps the invariant without over-freeing.
	 */
	 ...
	 * blocks. block_born/kill are skipped by the BPR, so dd_used is otherwise
	 * stale at the pre-promotion size; on `zfs destroy` that stale (too-small)
	 * dd_used is transferred to dp_free_dir while the async free drain
	 * (dsl_scan_free_block_cb) credits the real, larger block sizes PER BLOCK
	 * POINTER -- so a dedup'd block referenced N times is credited N times.
	 * The grow amount therefore differs by dataset kind:
	 *   - HEAD filesystem (dd_origin is 0 or the pool's hidden $ORIGIN): use
	 *     ref_delta, which counts EVERY reference (memo hit + miss). For a plain
	 *     head ref_delta == private_delta (one ref per block); for a deduped head
	 *     ref_delta is N x the block delta, matching the N per-BP credits at
	 *     destroy -- this is what fixes the dedup destroy `freeing` underflow.
	 *   - real CLONE (dd_origin is a user snapshot): use private_delta (memo miss
	 *     = clone-PRIVATE blocks only); blocks shared with the origin are the
	 *     origin's and are not freed with the clone, so must not be charged.
	 * Only physical (used) grows; logical comp/uncomp are unchanged by an
	 * in-place parity promotion. Snapshots fold into their fs via the deadlist
	 * and must NOT touch dd_used here.
	 */
...
/*
 * Deadlist remap. After every snapshot tree is relocated (memo fully populated),
 * rewrite every deadlist so no BP points at a relocated old block. A snapshot's
 * deadlist is a ZAP (ds_deadlist_obj) mapping mintxg -> sub-bpobj; each sub-bpobj
 * is an append-only BP list. A relocated block keeps its LOGICAL birth, so it
 * stays in the same mintxg bucket -- we rebuild each sub-bpobj with every BP
 * resolved through the memo, then swap it into the ZAP. This is what makes
 * `zfs destroy` of a promoted snapshot safe: destroy walks the deadlist and
 * every block it frees now exists at the promoted parity.
 */
typedef struct {
	bpobj_t *dr_newbpo;
	dmu_tx_t *dr_tx;
	spa_t *dr_spa;
	int64_t dr_used_delta;	/* growth in physical (asize) space from promotion */
} zhack_dlremap_ctx_t;
    ...
    dmu_tx_t *tx)
{
	...
	if (!BP_IS_HOLE(bp) && !BP_IS_EMBEDDED(bp)) {
		...
		if (m != NULL) {
			 ...
			 * PHYSICAL size (bp_get_dsize_sync). Promotion grows that
			 * (extra parity column), so track the delta and apply it
			 * to dl_used. dl_comp/dl_uncomp are logical -> unchanged.
			 */
			ctx->dr_used_delta +=
			    (int64_t)bp_get_dsize_sync(ctx->dr_spa, &m->bm_newbp) -
...
zhack_bpr_remap_dlobj(objset_t *mos, uint64_t dlobj, dmu_tx_t *tx)
{
	...
	/* hold the deadlist header (bonus) to correct dl_used for the asize growth */
	dmu_buf_t *hdr;
	...
	for (int i = 0; i < n; i++) {
		...
		(void) bpobj_iterate_nofree(&oldbpo, zhack_dlremap_itor, &ctx, NULL);
...
zhack_snap_dlremap_sync(void *arg, dmu_tx_t *tx)
{
	 ...
	 * dd_livelist (the ALLOC/FREE log of blocks born in the clone vs inherited
	 * from its origin) is SEPARATE from ds_deadlist_obj and still points at the
	 * pre-BPR block pointers. Pass 3 frees those old blocks, so destroying the
	 * clone would FREE THEM AGAIN through the livelist and underflow
	 * dp_free_dir: the pool's `freeing` counter wraps to ~2^64, which poisons
	 * dsl_pool_adjustedsize() so every later space-checked operation (notably
	 * destroying the clone-origin snapshot) fails ENOSPC -- surfaced to the
	 * user as the zcp wrapper error ECHRNG ("Channel number out of range").
	 * Remap the livelist through the SAME rewrite-once memo so every ALLOC/FREE
	 * entry points at the relocated block, exactly like the deadlist remap
	 * above (zhack_dlremap_itor preserves each entry's bp_freed flag). The
	 * livelist object is a deadlist-format object stored in the clone's dsl_dir
	 * ZAP under DD_FIELD_LIVELIST.
	 */
	...
	/*
	 * RE-DERIVE ds_unique_bytes from the now-consistent referenced + deadlist,
	 * using the standard identity (dsl_dataset_recalc_head_uniq generalized to
	 * any dataset):  unique = referenced - prev_snap.referenced + dl_used.
	 * ds_referenced was grown by the tree-BPR pass and dl_used by the deadlist
	 * remap, so this makes ds_unique consistent -- without it, block_kill's
	 * ASSERT(ds_unique_bytes >= used) underflows and panics on a promoted block.
	 */
	/*
	 * CLONE-ORIGIN GUARD (S4). The chain identity below (referenced - prev +
	 * dl_used) is only correct for a snapshot in a plain CHAIN, where every
	 * referenced-beyond-prev block is unique to this snapshot. A snapshot that
	 * is a CLONE ORIGIN (ds_next_clones_obj != 0) also shares blocks with its
	 * clone(s); those shared blocks are NOT unique and would be freed with the
	 * clone, not with this snapshot. The chain formula does not subtract them,
	 * so it OVER-counts ds_unique (empirically 0 -> full-referenced for a fully
	 * shared origin), which then makes `zfs destroy <origin-snap>` mis-compute
	 * its space reservation and fail ENOSPC. Pass 1 grew ds_referenced but left
	 * ds_unique_bytes untouched, so the value sitting here is still the ORIGINAL
	 * one ZFS derived correctly (accounting for the clone). For a clone origin
	 * we therefore PRESERVE it rather than re-derive from the chain identity.
	 */
	...
	if (dsl_dataset_phys(snap)->ds_prev_snap_obj != 0) {
		...
		if (dsl_dataset_hold_obj(dp,
		    dsl_dataset_phys(snap)->ds_prev_snap_obj, FTAG, &prev) == 0) {
 ...
 * PHYSICAL dedup sharing is preserved. But the DDT entry still points at the OLD
 * DVA. For each relocated deduped block, look its entry up by the (unchanged)
 * checksum and repoint the entry's phys DVAs + phys_birth to the new block,
 * preserving the refcount. ddt_lookup loads the entry into ddt_tree so ddt_sync
	...
	for (zhack_bpr_memo_t *m = avl_first(&zhack_bpr_memo); m != NULL;
		...
		if (dde != NULL && (dde->dde_flags & DDE_FLAG_LOADED) &&
			...
			ddt_phys_variant_t v = ddt_phys_select(ddt, dde,
			    &m->bm_oldbp);
			if (v != DDT_PHYS_NONE) {
				boolean_t flat = (ddt->ddt_flags & DDT_FLAG_FLAT);
				dva_t *dvas = flat ?
				...
				if (flat)
					...
					dde->dde_phys->ddp_trad[v].ddp_phys_birth =
					    BP_GET_BIRTH(&m->bm_newbp);
...
/*
 * P8b Pass 2.6: migrate the BRT (block-cloning reference table). BRT is keyed by
 * PHYSICAL DVA (vdev,offset), unlike the DDT (keyed by checksum). A cloned block has
 * one physical copy, M block pointers, and BRT bre_count = M-1 at its DVA (the base
 * reference is not counted). The rewrite-once memo repoints all M tree BPs to the new
 * DVA, but the new DVA is NOT in the BRT -- so on destroy each of the M frees would
 * physically free the SAME new block (zio_brt_free sees no entry -> frees), a
 * double-free that PANICS the metaslab. And the OLD DVA keeps a stale bre_count.
 * Migrate: establish the new DVA's bre_count = M-1 (brt_pending_add x (M-1)) and clear
 * the old DVA's (brt_entry_decref x (M-1)). Then on destroy the new block's M frees do
 * M-1 decrefs + one physical free; and Pass 3's single free of the OLD block (now
 * bre_count 0) frees it exactly once. BRT only tracks L0 data blocks (zio_brt_free
 * skips level>0 / metadata), so restrict to those.
 */
static void
 ...
 * deferred-free bpobj (dp_free_bpobj) so the block is freed through the KERNEL's
 * normal syncing free on the next import -- that path persists the free to the
 * on-disk spacemap (an offline zio_free from libzpool only updates the in-core
 * ms_allocatable, which is why earlier offline frees leaked).
 */
static void
zhack_bpr_free_sync(void *arg, dmu_tx_t *tx)
{
	...
	for (zhack_bpr_memo_t *m = avl_first(&zhack_bpr_memo); m != NULL;
		...
		if (BP_IS_HOLE(&m->bm_oldbp) || BP_IS_EMBEDDED(&m->bm_oldbp))
			continue;
		/*
		 * P8b: free the relocated OLD block as PLAIN. If it was a deduped
		 * block its DDT entry was already repointed to the new DVA
		 * (zhack_bpr_ddt_remap_sync), so freeing the old DVA must NOT go
		 * through ddt_phys_decref (that would decrement the migrated entry
		 * and could free the NEW block). Clearing the dedup bit routes the
		 * free through the plain metaslab path on the old DVA. Byte-neutral
		 * for non-dedup blocks (bit already 0).
		 */
		blkptr_t freebp = m->bm_oldbp;
		BP_SET_DEDUP(&freebp, 0);
		bpobj_enqueue(&dp->dp_free_bpobj, &freebp, B_FALSE, tx);
		/*
		 * The kernel's deferred-free drain (dsl_scan_free_block_cb) CREDITS
		 * dp_free_dir by the block's size when it frees it; charge dp_free_dir
		 * the same amount here so that credit balances (exactly how dsl_destroy
		 * charges dp_free_dir before enqueueing a block for async free).
		 */
...
zhack_do_snap_bpr(int argc, char **argv)
{
	...
	if (argc >= 2) {
		...
		/* S4 Part A: also process clones (live datasets with an origin). */
		(void) dmu_objset_find(argv[0], zhack_bpr_collect_clone, snaps,
		    DS_FIND_CHILDREN);
	}
	/*
	 * Collect (obj, creation_txg) and process OLDEST-first: with the
	 * rewrite-once memo, the oldest snapshot referencing a shared block owns
	 * it, so dsl_dataset_block_kill's free-vs-deadlist decision (keyed on the
	 * OLDER neighbour) is correct.
	 */
	...
	/* Pass 1: relocate + re-encode every snapshot tree (populates the memo). */
	...
	/*
	 * Pass 2: remap every snapshot's deadlist through the (now complete) memo,
	 * so no deadlist BP points at a relocated old block -- this is what makes
	 * `zfs destroy` of a promoted snapshot safe.
	 */
	for (int i = 0; i < nent; i++) {
		zhack_bpr_arg_t b;
		b.snap_obj = objs[i];
		(void) dsl_sync_task(spa_name(spa), NULL, zhack_snap_dlremap_sync,
		    &b, 0, ZFS_SPACE_CHECK_NONE);
	}
	for (int i = 0; i < 4; i++)
		txg_wait_synced(spa_get_dsl(spa), 0);
	/*
	 * Pass 2.5 (P8b): repoint the DDT so deduped blocks' dedup-table entries
	 * reference the relocated new DVAs (with the refcount preserved) BEFORE the
	 * old blocks are freed. No-op on non-deduped pools (the memo has no
	 * dedup-flagged old blocks).
	 */
	 ...
	 * relocated new-DVA, so cloned blocks stay refcounted at their new location
	 * (no double-free metaslab panic on destroy) and the old blocks free once.
	 * No-op if the block_cloning feature is inactive.
	 */
	...
	for (int i = 0; i < 4; i++)
		txg_wait_synced(spa_get_dsl(spa), 0);
	/*
	 * Pass 3: enqueue every relocated old block on the deferred-free queue and
	 * let it drain HERE. We charge dp_free_dir on enqueue (so the drain's credit
	 * balances) and MUST let the drain run in this same context, otherwise
	 * dsl_process_async_destroys' VERIFY0(dp_free_dir->dd_used_bytes) aborts the
	 * sync thread. The drain frees each old block through zio_free; the metaslab
	 * frees are persisted by the txg_wait_synced sync that follows.
	 */
	...
	(void) fprintf(stderr, "snap_bpr: rewritten=%llu dlremapped=%llu freed=%llu "
...
zhack_do_raidz_contract(int argc, char **argv)
{
	...
	/*
	 * Set the width epoch FIRST, while contracting/occmap are still OFF, so the
	 * epoch-set txg's own writes are clean width-N (the width-W gap check would
	 * mis-place them). T = last_synced + 2: that epoch-set txg (< T) stays
	 * width N, and every later sweep txg (>= T) is width W with the epoch
	 * already committed -- no within-txg width ambiguity (write width ==
	 * read width for every block, both keyed on physical birth).
	 */
	 ...
	 * whole-empty high metaslabs [W*child>>ms_shift, N); dropping a metaslab
	 * that straddles the ceiling (still holding valid data) panics
	...
	(void) fprintf(stderr, "raidz_contract: DONE T=%llu MOS dirtied=%llu "
	    "(import with raidz_contracting=1 + ceiling, then detach last child)\n",
	    (u_longlong_t)T, (u_longlong_t)zhack_mos_dirtied);
	spa_close(spa, FTAG);
	/*
	 * Export NOW, while routing + the gap allocator are still armed, so the
	 * final export sync writes the config/uberblock/objset-root as width-W and
	 * gap-placed. Otherwise libzpool's process-exit auto-export syncs them with
	 * contracting OFF -> the objset root is written width-W but with the
	 * EXPANDED map at an unconstrained (5-wide) offset, then re-read with the
	 * simple 4-wide map off-device -> import EIO (dsl_pool_init error=5).
	 */
