...
get_usage(zpool_help_t idx)
{
	...
	case HELP_CONDENSE:
		return (gettext("\tcondense [-t <type>] [-c | -w] "
		    ...
		    "\tcondense --types\n"));
	case HELP_REPARITY:
		return (gettext("\treparity [--online | --async | "
		    "--status | --cancel] <pool>\n"));
	case HELP_VERSION:
...
zpool_do_reparity(int argc, char **argv)
{
	...
	if (status) {
		...
		if (out != NULL) {
			...
			(void) nvlist_lookup_boolean_value(out,
			    "committed", &comm);
			(void) nvlist_lookup_boolean_value(out,
			    "canceling", &cing);
		...
		if (op == NULL || op[0] == '\0')
			(void) printf(gettext(
			    "no reparity operation on record\n"));
		else if (act)
			(void) printf(gettext("reparity of '%s' RUNNING%s: "
			    "target parity %llu, "
			    "visited %llu blocks, residual %llu\n"),
			    op, cing ? " (canceling)" : "", (u_longlong_t)tgt,
			    (u_longlong_t)vis, (u_longlong_t)res);
		else
			(void) printf(gettext("reparity of '%s' %s: target "
			    "parity %llu, visited %llu "
			    "blocks, residual %llu (err %d)\n"),
	...
	if (async) {
		...
		if (e == 0)
			...
			(void) fprintf(stderr, gettext("reparity of '%s' "
	...
	if (err == 0 && committed) {
		(void) printf(gettext("reparity of '%s' COMMITTED at parity "
		    "%llu (visited %llu blocks)\n"), pool, (u_longlong_t)target,
		    (u_longlong_t)visited);
	} else if (err == EBUSY) {
		(void) fprintf(stderr, gettext("reparity of '%s': an operation "
		    "is already in progress\n"), pool);
	} else {
		if (busy > 0)
			(void) fprintf(stderr, gettext("reparity of '%s': %llu "
			    "dataset(s) busy; unmount them first (zfs unmount "
			    "-a)\n"),
			    pool, (u_longlong_t)busy);
		else if (snaps > 0 && residual > 0)
			(void) fprintf(stderr, gettext("reparity of '%s' "
			    "blocked by %llu snapshot(s) pinning %llu blocks "
			    "at the old parity; destroy them and "
			    "re-run\n"), pool, (u_longlong_t)snaps,
			    (u_longlong_t)residual);
		else
			(void) fprintf(stderr, gettext("reparity of '%s' did "
			    "not complete: residual=%llu visited=%llu (err "
			    "%d)\n"), pool,
